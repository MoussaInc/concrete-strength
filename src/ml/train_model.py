...
ML_Cols = ['Cement', 'Slag', 'FlyAsh', 'Water', 'Superplasticizer', 'CoarseAggregate', 'FineAggregate', 'Age', 'Strength', 
...
def load_data_from_db(table_name: str) -> tuple:
    ...
    ml_set = set(ML_Cols)
    df = df.drop(columns=[c for c in df.columns if c not in ml_set])
...
def evaluate_model(model, X_test, y_test):
    """
    Évaluation du modèle avec RMSE et MAE.
    """
...
def make_pipeline(model):
    """
    Pipeline de ML incluant l'imputation des NaN (par la médiane) et la standardisation (scaling).
    """
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    t_end = time()
    logger.info(f"RandomForest entraîné en {round(t_end - t_start, 2)} secondes")
...
def main():
    logger.info("\n--- Démarrage de l'entraînement ML (Priorité Robustesse et Generalisation) ---")
    ...
    for name, info in results.items():
        logger.info(f"\nModèle: {name} | RMSE: {info['rmse']:.2f} | MAE: {info['mae']:.2f}")
